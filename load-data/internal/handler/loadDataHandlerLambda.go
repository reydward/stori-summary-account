...
func getPayload(request events.APIGatewayProxyRequest) (model.RequestPayload, error) {
	...
	for {
		...
		if err != nil {
			fmt.Printf("Could not read multipart data: %v", err)
			return model.RequestPayload{}, err
		}
		if part.FormName() == "transactionFile" {
			fileName = part.FileName()
			_, err = io.Copy(&fileContent, part)
			if err != nil {
				fmt.Printf("Could not read file content: %v", err)
				return model.RequestPayload{}, err
			}
		}
		if part.FormName() == "accountId" {
			...
			if err != nil {
				fmt.Printf("Could not read accountId: %v", err)
				return model.RequestPayload{}, err
			}
	...
	fmt.Printf("getPayload.accountIDNumber: %v", accountIDNumber)

	fileName = "test.csv"
	fmt.Printf("getPayload.fileName: %v", fileName)
